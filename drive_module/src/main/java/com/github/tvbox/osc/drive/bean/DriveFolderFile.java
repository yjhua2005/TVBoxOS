package com.github.tvbox.osc.drive.bean;
...
public class DriveFolderFile {
    ...
    public DriveFolderFile(DriveFolderFile parent, String name, int version, boolean isFile, String fileType, Long lastModifiedDate) {
        if (parent != null) {
            ...
            accessingPath = path.toArray(new String[0]);
        }
    ...
    public StorageDriveType.TYPE getDriveType() {
        if (driveData == null) throw new IllegalStateException("getDriveType() called on a non-drive item (driveData is null)");
    ...
    public String getWebDAVBase64Credential() {
        try {
            // [P2修复] 增加 config 非 null 检查，避免 NullPointerException
            if (config != null && config.has("username") && config.has("password")) {
