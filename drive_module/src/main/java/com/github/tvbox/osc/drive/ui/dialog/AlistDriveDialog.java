...
public class AlistDriveDialog extends DriveBaseDialog {
    ...
    public AlistDriveDialog(@NonNull @NotNull Context context, StorageDrive drive) {
        super(context);
        setContentView(R.layout.drive_dialog_alistdrive);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnConfirm).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // [P2修复] 统一使用 trim().isEmpty() 进行输入验证，与其他 Dialog 一致
                if (name.trim().isEmpty()) {
                    Toast.makeText(getContext(), getContext().getString(R.string.drive_alist_please_name), Toast.LENGTH_SHORT).show();
                    return;
                }
                // [P2修复] 统一使用 trim().isEmpty() 进行输入验证
                if (url.trim().isEmpty()) {
                    Toast.makeText(getContext(), getContext().getString(R.string.drive_alist_please_url), Toast.LENGTH_SHORT).show();
                    return;
                }
                if (!url.endsWith("/")) url += "/";
                ...
                if (drive != null) {
                    ...
                    DriveDataManager.insertDriveRecord(name, StorageDriveType.TYPE.ALISTWEB, config);
                }
